package de.darmstadt.tu.crossing.handler;
...
public class Parser {
		...
		for (final ParEqualsPredicate pred : pre_preds.keySet()) {
			...
			if (cond == null) {
				...
				actPreds.add(new CrySLCondPredicate(pred.getBaseObject(), pred.getPredName(), pred.getParameters(), pred.isNegated(),
						getStatesForMethods(ParserUtils.resolveAggregateToMethodeNames(cond))));
			}
		}
	...
	private List<CrySLForbiddenMethod> getForbiddenMethods(final EList<ForbMethod> methods) {
		...
		for (final ForbMethod fm : methods) {
			...
			if (alternative != null) {
				crysl.addAll(ParserUtils.resolveAggregateToMethodeNames(alternative));
			}
	...
	private ISLConstraint getPredefinedPredicate(final LiteralExpression lit) {
		...
		switch (pred) {
			case "callTo":
				...
				methodsToBeCalled.addAll(ParserUtils.resolveAggregateToMethodeNames(((PreDefinedPredicates) lit.getCons()).getObj().get(0)));
			...
			case "noCallTo":
				...
				final List<CrySLMethod> resolvedMethodNames = ParserUtils.resolveAggregateToMethodeNames(((PreDefinedPredicates) lit.getCons()).getObj().get(0));
	...
	private CrySLPredicate extractReqPred(final ReqPred pred) {
		...
		EObject con = innerPred.getCons();
		if (!(con instanceof Constraint)) {
			return null;
		}
		final Constraint conditional = (Constraint) con;
